...
import gov.usgs.cida.nude.column.impl.NudeRowImpl;
...
public class ResultSetUtils {
	...
	public static void spitOutStuff(ResultSet actual) throws SQLException {
		while(actual.next()) {
			NudeRow actualTR = NudeRowImpl.buildNudeRow(actual);
			
			StringBuilder sb = new StringBuilder();
			for (Column col : actualTR.getColumns()) {
				sb.append(actualTR.getString(col));
				sb.append(" ");
			}
	...
	public static boolean checkEqualRows(ResultSet expected, ResultSet actual) throws SQLException {
		...
		if (null != expected && null != actual) {
			...
			while(result && hasRows) {
				...
				if (hasRows) {
					NudeRow expectedTR = NudeRowImpl.buildNudeRow(expected);
					NudeRow actualTR = NudeRowImpl.buildNudeRow(actual);
	...
	public static Iterable<NudeRow> createNudeRows(ColumnGroup cg, String[][] rows) {
		List<NudeRow> result = new ArrayList<>();
		...
		if (null != cg) {
			...
			for (String[] row : rows) {
				...
				if (null != row && row.length == cg.size()) {
					Map<Column, String> rowMap = new HashMap<>();
					for (int j = 0; j < cg.size(); j++) {
						rowMap.put(cg.get(j+1), row[j]);
					}
					result.add(new NudeRowImpl(cg, rowMap));
				} else {
